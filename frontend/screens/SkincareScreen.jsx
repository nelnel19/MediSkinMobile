...
export default function SkincareScreen({ navigation }) {
    ...
    try {
      ...
      if (saveResponse.data.success) {
        if (saveResponse.data.isDuplicate) {
          Alert.alert("Already Saved", "This analysis has already been saved to your history.");
        } else {
          Alert.alert(
            "Saved Successfully",
    ...
    try {
      ...
      if (result) {
        if (result.error === "NO_FACE_DETECTED" || !result.face_detected) {
          ...
          setResult(result);
          console.log("Analysis complete:", result.skin_attributes);
        }
      } else {
  ...
  // Skin Tone Card using Face++ data (0-4 scale)
  const SkinToneCard = ({ skinTone }) => {
    if (!skinTone) return null;
    
    return (
      <View style={styles.skinToneSection}>
        <Text style={styles.sectionTitle}>Skin Tone (Face++)</Text>
        <View style={styles.skinToneCard}>
          ...
          <View style={styles.skinToneInfo}>
            ...
            <Text style={styles.skinToneDescription}>
              {skinTone.description || "Medium skin tone"}
            </Text>
          </View>
        </View>
      </View>
        ...
        <View style={styles.skinIssueContent}>
          ...
          <View style={styles.skinIssueProgress}>
            <View style={[
              ...
              { width: `${percentage}%`, backgroundColor: severityColor }
            ]} />
          </View>
        </View>
      </View>
              ...
              <View style={styles.resultsSection}>
                ...
                <View style={styles.skinIssuesSection}>
                  ...
                  {result.skin_attributes && (
                    <>
                      <SkinIssueCard title="Acne" value={result.skin_attributes.acne || 0} icon="flare" />
                      <SkinIssueCard title="Dark Spots" value={result.skin_attributes.stain || 0} icon="brightness-6" />
                      <SkinIssueCard title="Dark Circles" value={result.skin_attributes.dark_circle || 0} icon="remove-red-eye" />
                      {/* Blackhead card removed - not accurate from Face++ */}
                    </>
                  )}
                </View>
