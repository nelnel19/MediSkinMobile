...
export default function SkincareScreen({ navigation }) {
  ...
  const CameraScreen = () => {
    ...
    const takePicture = async () => {
      ...
      try {
        ...
        if (Platform.OS === 'ios' && cameraType === 'front') {
  ...
  // Simple Acne Location Text Component
  const AcneLocationText = ({ regionAnalysis }) => {
    if (!regionAnalysis) return null;
    
    const getAffectedAreas = () => {
      const areas = [];
      const acneData = regionAnalysis.acne_by_region;
      
      if (acneData.forehead > 30) areas.push("forehead");
      if (acneData.left_cheek > 30) areas.push("left cheek");
      if (acneData.right_cheek > 30) areas.push("right cheek");
      if (acneData.chin > 30) areas.push("chin");
      if (acneData.nose > 30) areas.push("nose");
    ...
    const getSeverityLevel = () => {
      const avgScore = (
        regionAnalysis.acne_by_region.forehead +
        regionAnalysis.acne_by_region.left_cheek +
        regionAnalysis.acne_by_region.right_cheek +
        regionAnalysis.acne_by_region.chin +
        regionAnalysis.acne_by_region.nose
      ) / 5;
      
      if (avgScore < 20) return "minimal";
      if (avgScore < 40) return "mild";
    ...
    if (affectedAreas.length === 0) {
      return (
        <View style={styles.acneLocationCard}>
          <Icon name="check-circle" size={20} color={COLORS.success} />
          <Text style={styles.acneLocationText}>
            No significant acne detected in any area
          </Text>
        </View>
      );
    }
    ...
    if (affectedAreas.length === 1) {
      message = `Acne is primarily detected on your ${affectedAreas[0]}`;
    } else if (affectedAreas.length === 2) {
      message = `Acne detected on your ${affectedAreas[0]} and ${affectedAreas[1]}`;
    } else {
      const lastArea = affectedAreas.pop();
      message = `Acne detected on your ${affectedAreas.join(", ")} and ${lastArea}`;
    }
    
    const severityMessages = {
      minimal: "(minimal presence)",
      mild: "(mild condition)",
      moderate: "(moderate concern)",
      significant: "(significant presence)"
    };
    ...
    return (
      <View style={styles.acneLocationCard}>
        ...
        <Text style={styles.acneLocationText}>
          {message} {severityMessages[severityLevel]}
        </Text>
      </View>
    );
  };
          ...
          >
            ...
            {result && (
              <View style={styles.resultsSection}>
                ...
                {/* Acne Location Text - Simple and clean */}
                {result.acne_regions && (
                  <AcneLocationText regionAnalysis={result.acne_regions} />
                )}
